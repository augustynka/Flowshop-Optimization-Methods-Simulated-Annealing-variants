...
int simulatedAnnealing(vector<Task>& tasks, const vector<Downtime>& downtimes,
    ...
    // EDD startowa permutacja
    sort(perm.begin(), perm.end(), [&](int a, int b) {
        return tasks[a].due < tasks[b].due;
    });
...
void runTestBlock(const string& label, const vector<int>& values, ofstream& out,
    ...
    for (int v : values) {
        ...
        //wielokrotnie na tej samej instancji
...
int main() {
    ofstream out("SA8_Lmax_EDD_no_reheat_54_0998.csv");
    out << "TypTestu,TestID,n,h,tau,Pmin,Pmax,Tight,Lmax,TimeUs\n";
    runTestBlock("n", {5,10,15,20,25}, out, 7,3,20,1,5,1.0);
    runTestBlock("h", {1,3,5,7,9}, out, 7,3,20,1,5,1.0);
    runTestBlock("tau", {10,20,30,40,50}, out,7,3,20,1,5,1.0);
    runTestBlock("range",{3,5,10,15,20},out,7,3,20,1,5,1.0);
    out.close();
    cout<<"SA6_Lmax_EDD_no_reheat.csv generated"<<endl;
    return 0;
}
