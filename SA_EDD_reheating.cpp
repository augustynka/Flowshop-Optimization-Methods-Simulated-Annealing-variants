...
int simulatedAnnealing(vector<Task>& tasks, const vector<Downtime>& downtimes,
                       int maxIter = 100000, double temp0 = 1000.0, double alpha = 0.998) {
    int n = tasks.size();
    vector<int> perm(n);
    iota(perm.begin(), perm.end(), 0);
    // EDD start
    sort(perm.begin(), perm.end(), [&](int a, int b) { return tasks[a].due < tasks[b].due; });
    ...
    int currL = bestL;
    int stagnation = 0;
    const int reheatingPeriod = maxIter / 5; // co 20% iteracji
    ...
    for (int iter = 1; iter <= maxIter; ++iter) {
        ...
        // reheating co określoną liczbę iteracji lub przy stagnacji
...
int main() {
    ofstream out("SA6_Lmax_results_15_0988.csv");
